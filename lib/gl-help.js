...
const loadProgram = async (gl, vertexPath, fragmentPath) => {
    const vertShader = loadShader(gl, gl.VERTEX_SHADER, vertexPath)
    const fragShader = loadShader(gl, gl.FRAGMENT_SHADER, fragmentPath)
    await Promise.all([vertShader, fragShader])
