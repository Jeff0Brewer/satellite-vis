...
const mouseRotate = (matrix, deltaX, deltaY, rotateSpeed, rotateBound) => {
    ...
    if (Math.abs(currXRotation + thisXRotation) > rotateBound) { thisXRotation = Math.sign(thisXRotation) * (rotateBound - Math.abs(currXRotation)) }
...
const scrollZoom = (matrix, deltaScroll, zoomSpeed, minZoom, maxZoom) => {
    ...
    if (newLen > maxZoom) { newPosition = vec3.scale(vec3.create(), newPosition, maxZoom / newLen) } else if (newLen < minZoom) { newPosition = vec3.scale(vec3.create(), newPosition, minZoom / newLen) }
